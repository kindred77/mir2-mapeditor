...
namespace Client
{
    ...
    //            Texture = new Texture(DXManager.Device, w, h, 1, Usage.None, Format.A8R8G8B8, Pool.Managed);
    //            stream = Texture.LockRectangle(0, LockFlags.Discard);
    ...
    //                MaskTexture = new Texture(DXManager.Device, w, h, 1, Usage.None, Format.A8R8G8B8, Pool.Managed);
    //                stream = MaskTexture.LockRectangle(0, LockFlags.Discard);

    //                decomp = Decompress(reader.ReadBytes(MaskLength));

    //                stream.Write(decomp, 0, decomp.Length);

    //                stream.Dispose();
    //                MaskTexture.UnlockRectangle(0);
    //            }
